/**
 * 南京师范大学全年宣传节点日历
 * 来源：高招全年宣传要点.xlsx + 原创主题推送选题.xlsx
 *
 * type 说明：
 *   festival  —— 传统节日 / 法定节假日 / 网络节点
 *   school    —— 学校专属事件（校庆、开学典礼、招生节点等）
 *   season    —— 节气 / 季节性内容
 *   recruit   —— 高招核心节点（招生简章、报名、录取等）
 *   youth     —— 青年文化 / 学子活动
 */
...
export const NJNU_CALENDAR_NODES: CalendarNode[] = [
  ...
  },
  {
    ...
    description: '南师大校庆日与教师节同为9月10日，是全年最具特色的情感节点，双节叠加对师范类高校尤为独特，可深度策划校史人物、师生情感、校友回忆等系列内容。',
  },
  ...
  // ── 4月 ──────────────────────────────────────────────────────
  {
    ...
    description: '传统节日，可结合缅怀先贤做校史人物专题，也可借踏青节点展示随园春色。',
  },
  {
  ...
  },
  {
    ...
    description: '五四青年节是展示青年力量的重要节点，可策划获奖青年专访、青年精神主题策划等。',
  },
  {
    id: 'may-countdown30',
    month: 5,
  ...
  },
  {
    ...
    title: '志愿填报季',
    type: 'recruit',
    topics: ['志愿填报攻略', '南师大各专业报考指南汇总', '招生咨询通道汇总', '十大理由系列（6.7-6.25）', '录取通知书查询方式汇总'],
  ...
  },
  {
    ...
    title: '新生大数据',
    type: 'school',
    topics: ['2024级本科新生大数据长图', '军训记忆图鉴', '辅导员介绍系列', '招办纳新·社团纳新'],
  ...
  // ── 10月 ──────────────────────────────────────────────────────
  {
    ...
    description: '国庆黄金节点，可发布爱国主题海报，结合国家级专业成就，或用"秋天第一杯奶茶"轻量创意内容。',
  },
  {
    id: 'oct-autumn',
    month: 10,
  ...
  // ── 11月 ──────────────────────────────────────────────────────
  {
    ...
    title: '保研·考研冲刺',
    type: 'youth',
    topics: ['保研系列·南师大保研去向揭秘', '考研冲刺·图书馆深夜灯光', '中国大学生自强之星标兵', '创新创业大赛报道'],
    description: '11月保研/考研关键节点，考研学子进入冲刺，可策划学习故事、保研去向、图书馆深夜等共情内容。',
  },
  {
  ...
  },
  {
    ...
    title: '冬日随园',
    type: 'season',
    topics: ['冬日随园雪景', '温暖冬季·随园冬季御寒指南', '创意冬日情书', '高考倒计时150天'],
    description: '冬日随园若遇雪景则是极佳传播素材，同时可策划温暖冬季系列，呼应新一轮招生备战周期。',
  },
  {
    id: 'dec-yearend',
    month: 12,
    ...
    title: '年终回顾',
    type: 'school',
    topics: ['年度十大新闻投票及结果', '年度十大关键词', '年终回顾长图', '专业解读·线下宣讲预告', '招生宣讲预告'],
    description: '年终总结是全年收官内容，可发布十大新闻、十大关键词，同时预热下一年招生宣传周期。',
  },
]

/**
 * 根据当前日期动态计算 daysLeft，并返回近 90 天内的节点（按时间排序）
 */
export function getUpcomingNodes(allNodes: CalendarNode[], today: Date = new Date(), limit = 6): CalendarNode[] {
  const result: (CalendarNode & { daysLeft: number })[] = []

  for (const node of allNodes) {
    // 用月份估算日期（取月中旬作为模糊节点的锚点）
    const rawDate = node.date
    let targetMonth = node.month
    let targetDay = 15 // 默认月中
    ...
    if (matchExact) {
      ...
      const matchInitial = rawDate.match(/(\d+)月初/)
      if (matchInitial) targetDay = 5
      const matchEnd = rawDate.match(/(\d+)月下旬/)
      if (matchEnd) targetDay = 25
      const matchMiddle = rawDate.match(/(\d+)月中旬/)
      if (matchMiddle) targetDay = 15
    }
